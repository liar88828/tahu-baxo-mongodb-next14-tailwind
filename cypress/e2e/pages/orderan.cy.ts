...
describe( 'template spec', () => {
  ...
  it.skip( "bisa melakukan beberapa aksi tombol pada page bank", () => {
    cy.wait( 500 )
  ...
  it.skip( "bisa membuat data di dalam form orderan", () => {
  ...
  it( "bisa membuat data orderan", () => {
    cy.visit( '/table/Semua' )
    cy.url().should( 'include', '/table/Semua' )
    cy.get( `[data-test="test-master"]` ).then( ( $el ) => {
      if( !$el.text().includes( 'Data is empty' ) ||
        $el.text().includes( 'create' ) &&
        $el.text().includes( 'Data is empty' ) ||
        !$el.text().includes( 'penerima test 1' )
      ) {
        cy.visit( '/orderan/create' )
        cy.url().should( 'include', '/orderan/create' )
  ...
  it( 'bisa mengubah data orderan ', () => {
    cy.visit( '/table/Semua' )
    cy.url().should( 'include', '/table/Semua' )
    cy.get( `[data-test="test-master"]` ).then( ( $el ) => {
      if(
        $el.text().includes( 'penerima test 1' ) ||
        !$el.text().includes( 'penerima update' ) ) {
  ...
  it( 'bisa menghapus data orderan ', () => {
    cy.visit( '/table/Semua' )
    cy.url().should( 'include', '/table/Semua' )
    cy.get( `[data-test="test-master"]` ).then( ( $el ) => {
      if(
  ...
  it( 'akan menghapus apa bila ada ', () => {
    ...
    cy.get( `[data-test="test-master"]` ).then( ( $el ) => {
      ...
      if( $el.text().includes( 'penerima test 1' ) ) {
        cy.get( '[data-test="penerima"]' )
          .contains( "Penerima" )
          .parent().within( () => {
          cy.get( '[data-test="test-search"]' ).type( 'penerima test 1' )
      ...
      if( $el.text().includes( 'penerima update' ) ) {
        cy.get( '[data-test="penerima"]' )
          .contains( "Penerima" )
          .parent().within( () => {
          cy.get( '[data-test="test-search"]' ).type( 'penerima update' )
